...
class IsValidEmail {
  ...
  public function __construct($email, $options = array()) {
    ...
    if (strlen($email) > 254) {$this->value = 0; return 0;};
    // if (strlen($email) > 254) $this->value = 0;
    ...
    if (strlen($bits['local']) > 64) {$this->value = 0; return 0;};
    if (strlen($bits['domain']) > 255) {$this->value = 0; return 0;};
    // if (strlen($bits['local']) > 64) $this->value = 0;
    // if (strlen($bits['domain']) > 255) $this->value = 0;
    ...
    if (strlen($bits['domain-literal'])) {
      ...
      if (preg_match("!^\[$IPv4_address_literal\]$!", $bits['domain'], $m)) {
        ...
        if (intval($m[1]) > 255) {$this->value = 0; return 0;};
        if (intval($m[2]) > 255) {$this->value = 0; return 0;};
        if (intval($m[3]) > 255) {$this->value = 0; return 0;};
        if (intval($m[4]) > 255) {$this->value = 0; return 0;};
      } else {
        ...
        while (1) {
          ...
          if (preg_match("!^\[$IPv6_comp\]$!", $bits['domain'], $m)) {
            ...
            if (count($groups) > 7) {$this->value = 0; return 0;};
            break;
          }
          ...
          if (preg_match("!^\[$IPv6v4_comp\]$!", $bits['domain'], $m)) {
            ...
            $folded = (strlen($a) && strlen($b)) ? "$a:$b" : "$a$b";
            $groups = explode(':', $folded);
            if (count($groups) > 5) {$this->value = 0; return 0;};
            break;
          }
      ...
      if ($options['public_internet']) {
        if (count($labels) == 1) {$this->value = 0; return 0;};
      }
      ...
      foreach ($labels as $label) {
        ...
        if (strlen($label) > 63) {$this->value = 0; return 0;};
        if (substr($label, 0, 1) == '-') {$this->value = 0; return 0;};
        if (substr($label, -1) == '-') {$this->value = 0; return 0;};
      }
      ...
      if ($options['public_internet']) {
        if (preg_match('!^[0-9]+$!', array_pop($labels))) {$this->value = 0; return 0;};
      }
    }
